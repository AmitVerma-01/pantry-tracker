...
const Item = () => {
  ...
  const { generateRecipe, currentRecipe, loading: recipeLoading, error: recipeError, clearRecipe } = useRecipes(user?.uid);
  ...
  const handleBatchDelete = async () => {
    ...
    const confirmed = window.confirm(
      `Are you sure you want to delete ${checkedItems.size} item${checkedItems.size > 1 ? 's' : ''}?`
    );
    ...
    try {
      const idsToRemove = allItems
        .filter(item => checkedItems.has(item.itemName))
        .map(item => item.id);
  ...
  const handleSelectAll = () => {
    if (checkedItems.size === items.length && items.length > 0) {
      ...
      setCheckedItems(new Set(items.map(item => item.itemName)));
    }
  };
  ...
  const handleGetRecipe = async () => {
    ...
    try {
      const result = await generateRecipe(Array.from(checkedItems));
      if (result.isCached) {
            ...
            {loading ? (
              ...
              items.map((item) => (
                <ItemComponent
                  ...
                  checked={checkedItems.has(item.itemName)}
